...
Everything else in the project asks for rows and never opens a
connection itself. That keeps SQL in one place and makes it easy to explain
during the demo that is models hold behaviour and database.py holds storage.
"""
...
# Three core tables plus a small `categories` lookup so category names are stored once instead of being repeated on every equipment row that is called normalisation.
SCHEMA = """
CREATE TABLE IF NOT EXISTS members (
    id        INTEGER PRIMARY KEY AUTOINCREMENT,
    name      TEXT    NOT NULL,
    email     TEXT    NOT NULL UNIQUE,
    joined_on TEXT    NOT NULL,                 __ ISO date, e.g. 2026-09-21
...
CREATE TABLE IF NOT EXISTS loans (
    ...
    returned_on  TEXT,                           __ NULL means "still out"
    renewals     INTEGER NOT NULL DEFAULT 0
);

__ Speeds up the two lookups the reports hammer: "open loans" and "who has what".
...
class Database:
    ...
    def _migrate(self):
        """
        Tiny schema upgrade meaning databases created before renewals existed lack the
